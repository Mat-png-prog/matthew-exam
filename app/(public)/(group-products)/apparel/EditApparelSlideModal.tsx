//app/(public)/(group-products)/apparel/EditApparelSlideModal.tsx
...
import { ApparelSlide } from "../types";

// Props for the EditApparelSlideModal component
interface EditApparelSlideModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  slide: ApparelSlide;
...
// EditApparelSlideModal component for editing an existing slide
const EditApparelSlideModal: React.FC<EditApparelSlideModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  slide,
  ...
  const { updateSlide, slides } = useApparelSlideStore();
  ...
  async function onSubmit(data: EditSlideFormValues) {
    try {
      setLoading(true);
      console.log("[EditApparelSlideModal] Submitting updated slide data (not exposing sensitive info)", { title: data.title, order: data.order });
      ...
      if (typeof window === "undefined") {
        ...
        console.error("[EditApparelSlideModal] Update error:", error);
      }
      toast.error(
...
export default EditApparelSlideModal;