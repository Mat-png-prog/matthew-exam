...
import {
  ALLOWED_IMAGE_TYPES,
  MAX_IMAGE_SIZE,
  type AllCollectionsSlide,
  type AllCollectionsSlideResponse,
} from "../../types";

// CREATE
export async function createAllCollectionsSlide(formData: FormData): Promise<AllCollectionsSlideResponse> {
  try {
    ...
    const slide = await prisma.allCollectionsBanner.create({
      data: {
        title,
        description,
        bgColor,
    ...
    console.error("Error creating all-collections slide:", error);
    return {
      success: false,
...
// READ (GET ALL)
export async function getAllCollectionsSlides(): Promise<{ success: boolean; data?: AllCollectionsSlide[]; error?: string }> {
  try {
    const slides = await prisma.allCollectionsBanner.findMany({
      orderBy: { order: "asc" },
    });
    return { success: true, data: slides };
  } catch (error) {
    console.error("Error fetching all-collections slides:", error);
    return {
      success: false,
      error: error instanceof Error ? error.message : "An unexpected error occurred",
    };
