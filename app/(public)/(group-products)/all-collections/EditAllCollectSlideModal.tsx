//app/(public)/(group-products)/all-collections/EditAllCollectSlideModal.tsx
...
import { editSlideSchema, type EditSlideFormValues } from "../../_components/(section-1)/validations";
import useAllCollectionsSlideStore from "./_store/all-collections-slide";
import Image from "next/image";
import { AllCollectionsSlide } from "../types";

// Props for the EditAllCollectionsSlideModal component
interface EditAllCollectionsSlideModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  slide: AllCollectionsSlide;
}
...
// EditAllCollectionsSlideModal component for editing an existing slide
const EditAllCollectionsSlideModal: React.FC<EditAllCollectionsSlideModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  slide,
}) => {
  // Local state for loading, preview image, and preview background color
  ...
  const { updateSlide, slides } = useAllCollectionsSlideStore();
  ...
  async function onSubmit(data: EditSlideFormValues) {
    try {
      setLoading(true);
      console.log("[EditAllCollectionsSlideModal] Submitting updated slide data (not exposing sensitive info)", { title: data.title, order: data.order });
      ...
      if (typeof window === "undefined") {
        ...
        console.error("[EditAllCollectionsSlideModal] Update error:", error);
      }
      toast.error(
...
export default EditAllCollectionsSlideModal;