//app/(public)/(group-products)/headwear/_actions/update-delete.ts
...
import { type HeadwearSlideResponse } from "../../types";

// UPDATE (with optional image replacement)
export async function updateHeadwearSlide(formData: FormData): Promise<HeadwearSlideResponse> {
  try {
    ...
    const currentSlide = await prisma.headwearBanner.findUnique({ where: { id } });
    ...
    console.error("Error updating headwear slide:", error);
    return {
      success: false,
...
export async function deleteHeadwearSlide(id: string): Promise<HeadwearSlideResponse> {
  try {
    const { user } = await validateRequest();
    if (!user) throw new Error("Unauthorized access");
    if (user.role !== "EDITOR" && user.role !== "SUPERADMIN") return redirect("/");
    ...
    const deletedSlide = await prisma.headwearBanner.delete({
      where: { id },
    });
    ...
    console.error("Error deleting headwear slide:", error);
    return {
      success: false,
      error: error instanceof Error ? error.message : "An unexpected error occurred",
    };
